...
class InitPopulation(object):
    ...
    def __init__(self, init, greedy, popsize_init):
        ...
        for x in [self.SteepestAscentSolution(self.InitializeRandomPopulation(self.greedy)) for _ in range(self.pop_size)]:
...
#                self.refresh(self.pop.chromosomes[mutate_index][1])
    ...
    def visualise(self, solution):
        ...
        for i, route in enumerate(solution.routes):
            ...
            b_c = i*20
            nodes = route.route
            norm = lambda x, y: ((20*x), (20*y))
            draw.line([norm(*self.pop.initial.coords[n]) for n in nodes], fill=(r_c, g_c, b_c), width=10)
        return im
...
#    data_file = ["A-n36-k5.vrp","A-n32-k5.vrp","A-n48-k7.vrp","A-n55-k9.vrp","A-n64-k9.vrp","B-n31-k5.vrp","B-n38-k6.vrp","B-n43-k6.vrp","B-n50-k7.vrp","B-n50-k8.vrp"]
    
    data_file = ["A-n36-k5.vrp"]
    running_time = 600
    running_iteration=10000
    replace_rate = [0.15]
    mutation_prob = [0.02]
    popsize = [200]
    greedy=True
